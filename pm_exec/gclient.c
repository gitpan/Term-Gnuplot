...
#else
# define input_from_PM_Terminal 0
#endif
...
/*fprintf(ff,"\n*** lRetCount=%i while nColors=%i\n",(int)lRetCount,(int)nColors);fclose(ff);} */
		...
		fprintf(stderr, "drv: creating palette with %d colors\n", smooth_colors);
...
/*		      GpiSelectPalette( hps, pm3d_hpal_old ); */
	       ...
	       case 'C': { /* GR_SET_COLOR */
			...
			fprintf(stderr, "drv: color %d requested\n", pm3d_color);
		    }
		    break ;
	       case 'f': { /* GR_FILLED_POLYGON */
		    ...
		    if (pm3d_color>=0) {
			GpiSetColor( hps, pm3d_color);
			fprintf(stderr, "drv: color set to %d\n", pm3d_color);
		    }
